...
for i, item in tqdm(enumerate(datasets['train']),total=len(datasets['train']), desc="Processing Questions"):
    ...
    outputs = lora_model.generate(input_ids=input_ids, max_new_tokens=config['max_new_token'],
                                  #do_sample=True, temperature=config['temperature'], top_p=config['top_p'], repetition_penalty=config['repetition_penalty'],
